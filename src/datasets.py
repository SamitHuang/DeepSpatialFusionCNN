...
class PatchWiseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with Image.open(self.names[im]) as img:
            ...
            # TODO: is using Image for processing computed in GPU? or CPU? why not use the lib torchvision.transform 
            if rotation != 0:
...
class ImageWiseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print("DEBUG: reading img:" + self.names[im])
        #if(self.names[im]=="./dataset_norm/train/Normal/n042.tif"):
        #    next
        with Image.open(self.names[im]) as img:
